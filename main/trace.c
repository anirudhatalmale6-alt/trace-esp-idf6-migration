...
extern FIFO_packet_t   FIFO_queue[]; // List
...
void trace_init(void)
{
  ...
  ICM45686_init(BMI270_CS);                      // Initialize the BMI270 accelerometer
...
void trace_loop(void *arg)
{
  ...
  while ( 1 )
  {
    DLT(DLT_TIMING, SEND(CONSOLE, sprintf(_xs, "Checking IMU interrupt");))
    if ( gpio_get_level(IMU_INTERRUPT) == 0 )
    {
      ICM45686_pull_FIFO();
      ICM45686_FIFO_statistics(); // Display the FIFO diagnostics
      FIFO_pull++;
